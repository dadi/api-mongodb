...
DataStore.prototype.getDatabaseOptions = function ({database, override} = {}) {
  ...
  const databaseName = (override && database)
    ? database
    : defaultDatabaseName
  const databaseBlock = databases[databaseName]

  if (!databaseBlock || !databaseBlock.hosts || !databaseBlock.hosts.length) {
    const legacyHostsBlock = config.get().hosts

    if (Array.isArray(legacyHostsBlock)) {
      const mainBlock = config.get()
      const exampleConfig = {
        database: defaultDatabaseName,
        databases: {
          [defaultDatabaseName]: Object.assign({}, mainBlock, {
            connectWithCollection: undefined,
            database: undefined,
            databases: undefined,
            enableCollectionDatabases: undefined,
            env: undefined
          })
        }
      }

      console.warn(`The current MongoDB configuration uses a \`hosts\` array at the root level. This syntax has been deprecated and will be removed in a future release. Please move your database configuration to the \`databases\` object.\n\n${JSON.stringify(exampleConfig, null, 2)}`)
...
DataStore.prototype.getConnectionString = function (database, {
  ...
  const hosts = hostsArray.map(({port = 27017, host}) => {
    return `${host}:${port}`
  }).join(',')
