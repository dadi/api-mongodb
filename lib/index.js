...
DataStore.prototype.find = function({query, collection, options = {}, schema}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (Array.isArray(query)) {
      ...
      const extendedOptions = Object.assign({}, options, this.getExtraOptionsByOperation('find'))

      debug('find in %s %o %o', collection, query, extendedOptions)

      this.database
        .collection(collection)
        .find(query, extendedOptions, (err, cursor) => {
          if (err) return reject(err)
...
DataStore.prototype.getExtraOptionsByOperation = function(operationName){
  const configKey = "extra_options_by_operation";
  const extraOptionsPerOperationConfig = this.config.has(configKey) ? this.config.get(configKey) : {};
  console.log({extraOptionsPerOperationConfig});
  return extraOptionsPerOperationConfig[operationName] || {}
}
